...
set_matrix <- function(len,wid,q){ #Assigns a value of {1,2} to every cell of the matrix
  ndata <- len*wid
  matrix(data=sample(c(1:q),ndata,replace=T),len,wid)
}

ising_lattice_MCMC <- function(len,wid,beta,nruns){
  ...
  for (a in 1:nruns){
    for (i in 1:len){
      for (j in 1:wid){
        ...
        prop <- sample(c(1:2),size = 1)
        E_new <- sum(prop==neighbours) #Computes the energy of a randomly selected colour
...
g <- (ising_lattice_MCMC(200,200,log(1+sqrt(2)),200))
...
image(1:200,1:200,isinghm(200,200,200,beta=log(1+sqrt(2))),
