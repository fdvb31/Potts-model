...
segmenting <- function(lattice,nruns,beta_,var,q,ncomplete){
  #Gets the necessary grid,block and neighbourhood structure
  wid <- height(lattice)
  len <- width(lattice)
  temp <- rep(0,len*wid)
  runs <- array(temp,c(len,wid))
  ...
  #Runs ncomplete Pott model simulates with each simulating having nruns iterations
  ...
  #Calculate the mode pixel value at each pixel
  temp <- rep(0,len*wid)
